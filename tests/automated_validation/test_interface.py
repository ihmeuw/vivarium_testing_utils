...
def test_verify_all(status: str, sim_result_dir: Path, mocker: MockFixture) -> None:
    ...
    passing_count = sum(len(inner) for inner in context.test_results.passing.values())
    failing_count = sum(len(inner) for inner in context.test_results.failing.values())
